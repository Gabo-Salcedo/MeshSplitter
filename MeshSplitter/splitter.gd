...
static func split_model(
	...
	if options.extract_meshes:
		var mesh_result = _extract_meshes(root, base_dir, model_name, structure_type)
		...
		if mesh_node.mesh:
			...
			match structure_type:
				FolderStructure.ORGANIZED:
					mesh_path = base_dir.path_join("mesh/%s.tres" % mesh_name)
				FolderStructure.FLAT:
					mesh_path = base_dir.path_join("mesh_%s.tres" % mesh_name)
				FolderStructure.SINGLE:
					mesh_path = base_dir.path_join("%s.tres" % mesh_name)
